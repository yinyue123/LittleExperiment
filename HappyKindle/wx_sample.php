	...
	public function responseMsg()
	{
		...
		if (!empty($postStr)){
			...
			if($event=="subscribe"){
				...
				$contentStr = $this->cmd("./bind.sh ".$openid);
			...
			if(!empty( $keyword ))//如果用户端微信发来的文本内容不为空，执行46--51否则52--53
			{
				...
				//$contentStr = $this->cmd($keyword);
				if(substr($keyword,0,1)=="@"){
					$contentStr = $this->cmd("./send.sh ".$openid." ".$keyword);
					//$contentStr=$keyword;
				}elseif(substr($keyword,0,1)=="+"){
					$contentStr = $this->cmd("./bind.sh ".$openid." ".$keyword);
				}elseif($keyword=="?" || $keyword=="？" || $keyword=="帮助"){
					$contentStr = $this->cmd("./help.sh");
				}elseif($keyword=="refresh"){
					$contentStr = $this->cmd("./refresh.sh");
				}else{
					$contentStr = $this->cmd("./find.sh ".$keyword);
				}
				//$contentStr.=$_GET["openid"]."|";
